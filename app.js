...
function renderPlanComidas(plan) {
  ...
  plan.forEach((dia, idx) => {
    ...
    card.className = 'dia-comida-card' + (idx === 0 ? ' open' : '');
    card.innerHTML = `
      <div class="dia-comida-header">
        <div class="dia-comida-info">
          <p class="dia-comida-fecha">${idx === 0 ? 'Hoy · ' : ''}${DIAS_SEMANA[idx]}</p>
...
function renderComidaCardInicio(comidasDelDia, contenedorId) {
  ...
  const proxima = comidasDelDia[0];
  const totalKcal = comidasDelDia.reduce((s, c) => s + c.macros.kcal, 0);
    ...
    <div class="inicio-card" onclick="cambiarPestana('comidas')">
      <div class="inicio-card-row">
        ...
        <div class="inicio-card-content">
          <p class="label">Próxima: ${proxima.label}</p>
      ...
      <div class="pills">
        ${comidasDelDia.map(c => `<span class="pill-mini">${c.label}</span>`).join('')}
      </div>
    </div>
  `;
}
...
function actualizarPantallaInicio() {
  ...
  renderComidaCardInicio(comidasHoy, 'inicio-comida-hoy');
